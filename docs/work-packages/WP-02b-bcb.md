...
Implemented `BcbSgsSource` in `src/econbase/sources/bcb_sgs.py` and `BcbFocusSource` in `src/econbase/sources/bcb_focus.py`, registered them in `src/econbase/sources/__init__.py`, created catalog files `catalog/br/bcb_sgs.yaml` and `catalog/br/bcb_focus.yaml`, and appended all 14 series IDs to `catalog/ids.txt`.

Verification:
- Offline tests: `uv run pytest tests/test_source_bcb.py` (21/21 passed) and full suite `uv run pytest` (152/152 passed).
- Linting/Formatting: `uv run ruff check .` and `uv run ruff format --check .` clean.
- Live APIs: `uv run econbase update --source bcb_sgs --source bcb_focus` succeeded with 14 series updated and 0 errors. `econbase check` reports 0 `no_data` series for these sources.
- Discontinued series noted: `bcb_sgs:7806` (Swap DI-Pré 360 days) was retired by Banco Central do Brasil in September 2019 (last period `2019-09-30`), causing `econbase check` to mark it as `stale`. All other 13 series are active and reporting `ok`.
